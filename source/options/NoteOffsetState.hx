...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var piss:BGSprite = new BGSprite('low quality', -614.4, -355, 1, 1);
		piss.scale.set(0.6, 0.6);
		piss.updateHitbox();
		add(piss);

		// Characters
		boyfriend = new Character(770, 100, 'bf-normal', true);
		boyfriend.x += boyfriend.positionArray[0];
		boyfriend.y += boyfriend.positionArray[1];
		add(boyfriend);

		var cum:BGSprite = new BGSprite('forefog', -614.4, -355, 1.1, 0.15);
		cum.scale.set(0.6, 0.6);
		cum.updateHitbox();
		add(cum);
		// Combo stuff
